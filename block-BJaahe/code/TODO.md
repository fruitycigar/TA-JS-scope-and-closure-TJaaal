...
- `setLastName`: accepts a parameter last name using which updates the last Name and return the updated full name
...
function nameFactory(fn, ln) {
  return {
    ...
    setFirstName: function(firstn) {
      return `${firstn} ${ln}`;
    },
    setLastName: function(lastn) {
      return `${fn} ${lastn}`;
    }
  }
}
